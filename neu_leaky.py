...
# get data
df = pd.read_csv('neuroblastoma.csv')

X = df.iloc[:, 2:]  # X are all except Event-free
y = df['Event-free']  # Event-free as label
...
train_loader = DataLoader(dataset=train_dataset, batch_size=246, shuffle=True)
test_loader = DataLoader(dataset=test_dataset, batch_size=246, shuffle=False)

# NN
class MLP(nn.Module):
    def __init__(self, input_size = 10707, hidden_size=1000, output_size=2, weight_variance=None, activation='leaky_relu',
        ...
        # Initialize output layer weights with a specific value
...
def main_loop(weight_variances, num_epochs, train_loader, test_loader, device):
    ...
    for weight_variance in weight_variances:
        ...
        model = MLP(hidden_size=1000, weight_variance=weight_variance, activation='leaky_relu').to(device)
        optimizer = optim.SGD(model.parameters(), lr=0.22, momentum=0)
        rank_record = []
...
num_runs = 1
num_epochs = 500  # Change this to the desired number of epochs
...
plot_stable_ranks_and_test_errors_from_runs(all_rank_records, all_test_error_records, 'neu_stable_ranks_and_test_errors_leaky_relu.png')
...
# Save the records
save_records(all_rank_records, all_test_error_records, 'neu_rank_records_leaky_relu.pickle', 'neu_test_error_records_leaky_relu.pickle')
